        ...
        if isinstance(node, InternalNode):
            ...
            if not isRec:
                if isinstance(node.left_child, LeafNode):
                    total_wordvectors_grad.gradvectors[:, node.left_child.index] += dot(delta_parent, self.Wi1)

                if isinstance(node.right_child, LeafNode):
                    total_wordvectors_grad.gradvectors[:, node.right_child.index] += dot(delta_parent, self.Wi2)
